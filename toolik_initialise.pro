...
;\\ MIRROR ROUTINES
pro toolik_mirror,  drive_to_pos = drive_to_pos, $
...
pro toolik_filter, filter_number, $
					    log_path = log_path, $
				  	    misc, $
				  	    log
...
pro toolik_etalon, dll, $
	...
	if ((leg1_voltage eq leg2_voltage) and (leg1_voltage eq leg3_voltage)) then begin

		;\\ This is a normal scan (for Toolik, there are no leg 'gains')
		...
		;\\ This must be a parallelism adjustment (Etalonspacer)
		print, leg1_voltage, leg2_voltage, leg3_voltage

		xdiff = leg1_voltage - leg2_voltage
		ydiff = leg2_voltage - leg3_voltage

		sdi_cs100_etalon_driver, 'set_x_parallelism', {port:misc.port_map.etalon.number, $
												dll:dll, $
												comms:'moxa', spacing:xdiff}, out, err

		sdi_cs100_etalon_driver, 'set_y_parallelism', {port:misc.port_map.etalon.number, $
												dll:dll, $
												comms:'moxa', spacing:ydiff}, out, err
...
pro toolik_initialise, misc, console
	...
	;\\ Etalon Init
		...
		sdi_cs100_etalon_driver, 'set_x_parallelism', {port:misc.port_map.etalon.number, $
												dll:dll, $
												comms:'moxa', spacing:-130}, out, err
		sdi_cs100_etalon_driver, 'set_y_parallelism', {port:misc.port_map.etalon.number, $
												dll:dll, $
												comms:'moxa', spacing:-60}, out, err
