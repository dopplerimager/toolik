...
pro SDI_CS100_Etalon_Driver_GUI_Scan, start=start, stop=stop
	...
	if not keyword_set(stop) and not keyword_set(start) then begin
		...
		if temp_z lt -2048 or temp_z gt 2047 then begin
			misc.scan.scanning = 0
			return
		endif

		if temp_z ge misc.scan.zmin and temp_z le misc.scan.zmax then $
		...
		if temp_z ge misc.scan.zmax then begin
			wset, gui.bar
			loadct, 39, /silent
			erase, 0
			misc.scan.current_z = misc.scan.zmin
		endif
	endif
...
pro SDI_CS100_Etalon_Driver_GUI, dll, $
	...
	if size(dll, /type) eq 0 then $
		dll = 'C:\Users\sdi3000\sdi\bin\sdi_external.dll'
