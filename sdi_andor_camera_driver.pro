...
pro SDI_Andor_Camera_Driver, dll, command, in, out, result, unload=unload, auto_acq=auto_acq
	...
	case strlowcase(command) of
		...
		;\\ in = {mode:5, imageMode: from the settings structure}
		...
		;\\ Set up the acquisition using a settings tructure defined above
		...
		strlowcase('uApplySettingsStructure'): begin
			...
			SDI_Andor_Camera_Driver, dll, 'uSetAcquisitionMode', in.acqMode, thisout, res
			result = [result, 'uSetAcquisitionMode: '+res]
			out = [out, thisout]

			SDI_Andor_Camera_Driver, dll, 'uSetFrameTransferMode', in.frameTransfer, thisout, res
			result = [result, 'uSetFrameTransferMode: '+res]
			out = [out, thisout]
